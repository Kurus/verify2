...
for cur_ly in range(0,num_layer):
    ...
    # ker_l_1[0,0]=1
    ker_l_1 =np.random.randint(255,size=ker*dep, dtype='uint8').reshape((ker,dep))
    ...
    ker_l_3 = np.random.randint(255,size=ker*dep*9, dtype='uint8').reshape((ker,dep,9))
    ...
    if stride2_en==1:
        ...
        out_1 = np.zeros(ker*dep*o_dim*o_dim, dtype='uint8').reshape((ker,dep,o_dim,o_dim)) # output of exp 1 is zeros
        for k in range(0,ker):
            for l in range(0,dep):
                kk = ker_l_3[k,l]
